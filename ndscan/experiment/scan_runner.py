...
class KernelScanRunner(ScanRunner):
    ...
    # We store the param stores as attributes, divided by type
    # this is only possible because we limit the allowable param stores
    _float_stores: Kernel[list[FloatParamStore]]
    _int_stores: Kernel[list[IntParamStore]]
    _bool_stores: Kernel[list[BoolParamStore]]
    ...
    def setup(
        ...
        self._float_stores: list[FloatParamStore] = []
        self._int_stores: list[IntParamStore] = []
        self._bool_stores: list[BoolParamStore] = []

        # Divide parameter stores by type and add as attributes
        for axis in self._axes:
            store = axis.param_store
            if store.RpcType is float:
                self._float_stores.append(store)
            if store.RpcType is int32:
                self._int_stores.append(store)
            if store.RpcType is bool:
                self._bool_stores.append(store)
    ...
    def build_generated(self):
        self._fragment.build_generated(self.handler)
    ...
    @rpc
    def _get_param_values_chunk(self) -> tuple[int32, list[float], list[int32], list[bool]]:
        ...
        # To get around limitations in the type system, we send values divided in buckets
        # by type instead of by axis, thankfully stores are divided by type on the
        # kernel side as well.
        #
        # Kernel side needs to know the stride, this adds some slight overhead to each rpc
        # call but is hopefully acceptable. This may only be <CHUNK_SIZE at end of scan.
        values: tuple[int32, list[float], list[int32], list[bool]] = (
            int32(len(self._current_chunk)), [], [], []
        )

        for p in self._current_chunk:
            for value, axis in zip(p, self._axes):
                val = axis.param_store.to_rpc_type(
                    axis.param_store.coerce(axis.param_store.value_from_pyon(value))
                )
                if axis.param_store.RpcType is float:
                    values[1].append(val)
                if axis.param_store.RpcType is int32:
                    values[2].append(val)
                if axis.param_store.RpcType is bool:
                    values[3].append(val)
        return values
