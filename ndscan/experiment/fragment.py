...
class AggregateExpFragment(ExpFragment):
    r"""Combines multiple :class:`ExpFragment`\ s by executing them one
    ...
    forward to the parent implementation after constructing all the relevant fragments::
        ...
        class FooBarAggregate(AggregateExpFragment):
            def build_fragment(self):
                ...
                AggregateExpFragment.build_fragment(self, [self.foo, self.bar])
    ...
    def build(self, fragment_path: list[str], *args, **kwargs):
        ...
        self.run_once_behavior = self._run_once_impl

    def build_fragment(self, operands: list[ExpFragment]) -> None:
        """
        :param operands: The list of child fragments to be aggregated. The operands
            will be run in the given order, calling ``run_once`` on child fragments.
        """
        if not operands:
        ...
        self._run_once_impl = "\n".join(
            [f"self.{frag._fragment_path[-1]}.run_once()"
             for frag in self._exp_fragments]
        )

        is_kernels = [is_kernel(frag.run_once) for frag in self._exp_fragments]
        if any(is_kernels) and not all(is_kernels):
            raise Exception("Mixed host/@kernel functions are not supported")
